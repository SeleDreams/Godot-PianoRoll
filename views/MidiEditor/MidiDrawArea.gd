...
func clear_background():
	draw_rect(Rect2(Vector2.ZERO,Vector2(draw_width,draw_height)),Color(40 as float / 255,40 as float / 255,40 as float / 255,1))
...
func draw_beats():
	...
	for i in range(1,bars * timesig_numerator):
			...
			if beat_pos.x < rect_size.x and beat_pos.x > 0:
				draw_line(beat_pos,beat_pos + Vector2(0,draw_height),Color.gray)
...
func draw_bars():
	...
	for i in range(1,bars + 1):
		...
		if bar_pos.x < rect_size.x and bar_pos.x > 0:
			draw_line( bar_pos,bar_pos + Vector2(0,draw_height),Color.white,2.0)

func draw_octaves():
	for i in range(1,number_octaves * octave_notes + 1):
		var octave_pos = Vector2(0,space_between_notes * i) - Vector2(0,vertical_offset)
		var line_size : int = 1 if i % octave_notes != 0 else 2
		draw_line(octave_pos,octave_pos + Vector2(draw_width,0),Color.white,line_size)
...
func update_zoom():
	...
	var bpm_factor = 100 as float / bpm
	total_horizontal_size = (space_between_bars * bars / timesig_denominator * bpm_factor) + 2
